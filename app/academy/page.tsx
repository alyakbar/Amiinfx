// app/academy/page.tsx (or wherever your Academy page is)
"use client";
...
export default function AcademyPage() {
  ...
  // Map human-readable course name to the checkout slug your checkout page expects
  const slugMap: Record<string, string> = {
    "Online Masterclass": "online-masterclass",
    "In-Person Masterclass": "inperson-masterclass", // matches your SERVICES id
  };

  const handleApply = (courseType: string, price: number) => {
    setSelectedCourse(courseType);

    // build slug (fallback to slugified courseType)
    const slug = slugMap[courseType] ?? courseType.toLowerCase().replace(/\s+/g, "-");

    // navigate to checkout/<slug> and include price as query param
    router.push(`/checkout/${slug}?price=${encodeURIComponent(String(price))}`);
  };
            ...
            <div className="space-y-6">
              ...
              <Card className="bg-gray-800 border-gray-700">
                <CardContent className="p-6">
                  ...
                  <Button
                    onClick={() => handleApply("Online Masterclass", 250)}
              ...
              <Card className="bg-gray-800 border-gray-700">
                <CardContent className="p-6">
                  ...
                  <Button
                    onClick={() => handleApply("In-Person Masterclass", 500)}
